...
base_host_agent(Agent1) ->
	...
	{start_container,Sender, CTID,Req} ->
            %%Start container with given CTID
	    Command="vzctl start ",
	    Containerid=integer_to_list(CTID),
	    Execute=[Command]++Containerid,
	    ReturnVal=os:cmd(Execute),
	    Sender!{Req,ReturnVal},
            base_host_agent(Agent1);	

	{stop_container,Sender, CTID,Req} ->
	...
	{get_hostname,Sender,CTID,Request_id}->
 	    Container_id=CTID,
	    %io:format("Base agent ~p received get_hostname request from Sender ~p for CTID ~p with Request_id ~p~n", 
 	    %Container_id=CTID,
